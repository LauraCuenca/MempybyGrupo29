...
def procesar_fifa(ruta="src/recursos/datasets/fifa20data.csv", modo=1):
    """
    ...
    if modo==1:
        ...
        # Devuelve todos los jugadores con imagen, extranjeros jugando en Boca o River (13)
        jugadores = list(filter(lambda x: x if x[1] and x[2]!="Argentina" and (x[7]=="Boca Juniors" or x[7]=="River Plate") else None, datos))
        print(jugadores[0])
        print(len(jugadores))
...
def get_tabla_criterios():
    criterios = {'lunes': {(0, 12): {"criterio": "Pokemones tipo Grass y Poison", 
                ...
                'martes': {(0, 12): {"criterio": "Pokemones tipo Bug y diferentes a Poison o Flying", 
                            ...
                            (13, 23): {"criterio": "Pokemones tipo Dragon y que no tengan segundo tipo", 
                                    "funcion": procesar_pokemon, "modo": 4 }
                },
                'miercoles': {(0, 12): {"criterio": "Pokemones que contengan la letra k en su nombre, que la longitud de su nombre sea mayor a 8 y que solo tengan 1 tipo", 
                                    "funcion": procesar_pokemon, "modo": 5 },
                            (13, 23): {"criterio": "Pokemones que terminen su nombre en on", 
                                    "funcion": procesar_pokemon, "modo": 6 }
                },
                'jueves': {(0, 12): {"criterio": "Jugadores con imagen, pais==Argentina, edad entre 30 y 35 años que sean zurdos y pesen menos de 170lbs", 
                                    "funcion": procesar_fifa, "modo": 1 },
                            (13, 23): {"criterio": "Jugadores con imagen, edad menos a 18 años y jueguen de arquero", 
                                    "funcion": procesar_fifa, "modo": 2 }
                },
                'viernes': {(0, 12): {"criterio": "Jugadores con imagen, edad==23, Overall==Potential y que sean zurdos", 
                                    "funcion": procesar_fifa, "modo": 3 },
                            (13, 23): {"criterio": "Jugadores con imagen, cuyos nombres sean mas de 18 caracteres y contengan las letras k,z y w", 
                ...
                'sabado': {(0, 12): {"criterio": "Jugadores con imagen, edad menor a 25 y equipo barcelona", 
                            ...
                            (13, 23): {"criterio": "Jugadores con imagen, extranjeros jugando en Boca o River", 
                                    "funcion": procesar_fifa, "modo": 6 }
                },
                'domingo': {(0, 12): {"criterio": "Logos con imagen, con origen distinto a estados unidos o reino unido", 

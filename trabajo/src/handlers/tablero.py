...
def procesar_fifa(ruta="src/recursos/datasets/fifa20data.csv", modo=1):
    ...
    if modo == 1:
        ...
        jugadores = list(filter(
            lambda x: x if x[1] and x[2] != "Argentina" and (x[7] == "Boca Juniors" or x[7] == "River Plate") else None,
            datos))
        print(jugadores[0])
        print(len(jugadores))
...
def get_tabla_criterios():
    criterios = {'lunes': {(0, 12): {"criterio": "Pokemones tipo Grass y Poison",
                 ...
                 'miercoles': {(0, 12): {
                     "criterio": "Pokemones que contengan la letra k en su nombre, que la longitud de su nombre sea mayor a 8 y que solo tengan 1 tipo",
